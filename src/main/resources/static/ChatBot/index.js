...
function createPropertyCard(prop) {
  return `
    <li class="property-item" data-title="${prop.name}">
...
// 카드 클릭 이벤트 위임 추가
hotListContainer.addEventListener('click', function(e) {
  const card = e.target.closest('.property-item');
  if (!card) return;
  const title = card.getAttribute('data-title');
  // 온라인 임장으로 이동 (매물명 파라미터 전달)
  window.location.href = `/Design/index.html?title=${encodeURIComponent(title)}`;
});
...
//이달의 지역별 핫한 Zip - 온라인 임장 연결
window.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.hot-section .property-item').forEach(card => {
    card.addEventListener('click', function() {
      const title = this.querySelector('.property-title').textContent.trim();
      window.location.href = `/Design/index.html?title=${encodeURIComponent(title)}`;
    });
  });
});
